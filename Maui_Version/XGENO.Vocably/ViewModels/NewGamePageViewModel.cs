...
public partial class NewGamePageViewModel : AppViewModelBase
{
    [ObservableProperty]
    private Level theGame;

    public event EventHandler<WordEventArgs> WordLoadedEvent;

    public NewGamePageViewModel(IApiService appApiService, ISettingsService appSettingsService, IDatabaseService appDBService)
    ...
    public async Task<List<Level_Progress>> ProgressLevels() =>
        await _appDBService.GetLevelProgress(TheGame.Level_No);
