...
namespace XGENO.Wordly.Views
{
    public partial class NewGamePage : ViewBase<NewGamePageViewModel>
    {
        ...
        private async void InitializeGameScreen()
        {
            ...
            for (int i = 0; i < 6; i++)
            {
                //var wordleRow = WordleGrid.FindByName<StackLayout>($"WordleRow{i}");
                var wordleRow = WordleGrid.Children[i] as StackLayout;
                    ...
                    for (int i = 0; i < 5; i++)
                    {
                        ...
                        switch (row.ResultPhrase[i])
                        {
                            ...
                            case 'W':
                                ...
                                labelControl.TextColor = Color.Black;
            ...
            for (int i = 0; i < 6; i++)
            {
                ...
                for (int j = 0; j < 5; j++)
                {
                    ...
                    _ = wordleFrame.FadeTo(1, 1200, Easing.BounceOut);
                    await Task.Delay(40);
                }
            }
            ...
            _ = stkKeyboardRow1.FadeTo(1, 1200, Easing.SinIn);
            _ = stkKeyboardRow2.FadeTo(1, 1200, Easing.SinIn);
            await stkKeyboardRow3.FadeTo(1, 1200, Easing.SinIn);
        ...
        private async void Enter_Keyboard_Button_Clicked(object sender, EventArgs e)
        {
            ...
            if (theWord.ToLower() == guessWord.ToLower())
            {
                ...
                await PopupNavigation.Instance.PushAsync(animPopup);
        ...
        private Label GetLabelControl(WordleBox frame)
        {
            var label = frame.FindByName<Label>("lblLetter");
        ...
        {
            var showHont = await viewModel.Can_Show_Hint();

            //Show Hint
            if (showHont)
            {
                var hintPopup = new ShowHintPopupView(theMeaning, theWord);
                await PopupNavigation.Instance.PushAsync(hintPopup);
            }


        }

        private async void SetHighlightedBox(bool remove = false)
        {
            if (currentRow < 0 || currentRow > 5 || currentColumn < 0 || currentColumn > 4)
                return;
